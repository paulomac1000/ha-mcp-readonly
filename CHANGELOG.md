...
- The context generator no longer falls back to an implicit Home Assistant host: when `HA_URL` is unset, network collection stays disabled even when `HA_TOKEN` is set, so the token can never be transmitted to an implicit plain-HTTP endpoint. Deployments that want API sources set `HA_URL` explicitly.
