...
- New unit coverage for section/profile resolution, overflow policy resolution, budgeted staged rendering (including streaming above the copy chunk size and UTF-8 budget boundaries), artifact digest, configuration and REST option normalization (including alias-conflict and upper-bound rejection), the provenance policy-skip split, the artifact timestamp contract, and the public generation-result contract.
...
### Fixed
- Malformed JSON bodies to `POST /api/context/generate` now return HTTP 400 instead of being treated as an empty defaults request.
- The context status response distinguishes terminal failure classes through a stable `error_code` (`DEADLINE_EXCEEDED`, `INVALID_ARGUMENTS`, `GENERATION_FAILED`, `DEPENDENCY_UNAVAILABLE`, `INTERNAL`) instead of collapsing every failure into a generic message.
