...
### Security
- `make_ha_request` refuses to send the Home Assistant bearer token over cleartext HTTP to non-local destinations (CWE-319): HTTP remains allowed for loopback and private/link-local/unique-local addresses, and for hostnames that resolve exclusively to such addresses. Every hostname — single-label, mDNS-style, and fully qualified names alike — is resolved fresh on every request, all returned addresses must be private, and the request is pinned to the validated address with the original authority preserved in the `Host` header. Public cleartext destinations fail with `INSECURE_TRANSPORT` before any request is sent. HTTPS destinations are unaffected. Deployments serving Home Assistant over public cleartext HTTP must switch to HTTPS.
...
- A repository dependency-contract check keeps the runtime ranges in `requirements.txt` identical to `pyproject.toml`, so the documented install path cannot drift from the enforced FastMCP 3.4.x hold-back (protocol suite).
...
### Added
- Budget-aware context generation at the source (issue HAMCP-69EA3F-1). The context generator accepts generation options `profile` (`full`, `agent`, `compact`), `max_output_bytes` (byte budget), an explicit `include_sections` selection with alias support (`runtime`, `health`, `provenance`, `logs`), boolean source-category switches `include_repository_files` and `include_storage_records`, and the overflow policy `on_budget_exceeded` (`auto`, `fail`, `truncate`; `auto` keeps the historical fail-closed behavior for the default `full` profile and truncates whole sections for `agent`/`compact`). Environment equivalents: `HA_CONTEXT_PROFILE`, `HA_CONTEXT_SECTIONS`, `HA_CONTEXT_DETAIL`, `HA_CONTEXT_INCLUDE_FILES`, `HA_CONTEXT_INCLUDE_STORAGE`, `HA_CONTEXT_ON_BUDGET_EXCEEDED`. The REST `POST /api/context/generate` endpoint accepts `profile`, `maxBytes`, `sections`, `repositoryFiles`, `storageRecords`, `include_files`, `detail`, and `onBudgetExceeded`, rejecting conflicting alias values and out-of-range byte budgets with HTTP 400. Explicit section selections always include the mandatory floor (executive summary and source provenance) so every artifact keeps its completeness record.
- Generation results explicitly report `output_bytes`, `uncompressed_bytes`, `output_sha256`, `profile`, `profile_revision`, `max_bytes`, `generated_at`, `requested_sections`, `selected_sections`, `rendered_sections`, `omitted_sections` (with exact per-section byte sizes and reasons), and `truncated`, so consumers can verify what they received. `selected_sections` reflects the effective selection including the mandatory floor. Rendering stages each section through a disk-spooling buffer and streams accepted sections to the artifact in bounded chunks, so staging never retains an additional fully encoded section in memory. Budget-truncated artifacts carry an in-artifact Generation Notes section listing every omitted section with its exact staged size and retrieval guidance, and the source provenance section reports collection-scope completeness (the source completeness value never claims artifact completeness for budget omissions). The artifact format version constant is bumped accordingly. The mandatory floor fails the run explicitly when it cannot fit.
...
- New unit coverage for section/profile resolution, overflow policy resolution, budgeted staged rendering (including streaming above the copy chunk size and UTF-8 budget boundaries), artifact digest, configuration and REST option normalization (including alias-conflict and upper-bound rejection), the provenance policy-skip split, the artifact timestamp contract, the public generation-result contract, sensitive-canary absence under every profile, last-known-good download after a failed regeneration, a representative large-instance regression (realistic entity/device/area registries and structured automations) proving the operational profile stays bounded under a documented byte cap, and a large-source regression proving a legacy >50 MB context now yields a bounded artifact with explicit omissions.

### Changed
- Context artifacts timestamp their header and footer with timezone-aware ISO 8601 values instead of naive local time, and the artifact format version is owned by a single `CONTEXT_FORMAT_VERSION` constant instead of separate hard-coded strings.
- The REST OpenAPI document now describes the four `/api/context/*` routes alongside the tool routes.
- Update pinned GitHub Actions revisions from Dependabot PR #36 while preserving the current publish build-backend validation.
- Adopt the pip-group updates from Dependabot PR #35 except `fastmcp`, `fastmcp-slim`, `mcp`, and `starlette`, which keep their previous pins: with the candidate stack (FastMCP 4.0.1, MCP 2.1.1) installed, the hosted CI protocol suite fails its negotiated-protocol-revision check (run 34470052947), and the repository's Dependabot guard intentionally holds MCP 2.x back while FastMCP 3.4.x requires `mcp<2`. The `pydantic-core` constraint stays removed so Pydantic's exact dependency remains authoritative.
- Align the CI build-tooling pins (`build`, `setuptools`, `wheel`) with the updated constraints across `ci.yml`, `migration-evidence.yml`, `official-client.yml`, and `publish.yml`, resolving a `ResolutionImpossible` conflict introduced by the bump.
- Add an explicit `httpx` development dependency: `scripts/verify_runtime_endpoints.py` and `scripts/verify_official_mcp_client.py` import it directly, and it is no longer guaranteed transitively.
...
- The generation worker no longer parses budget-related environment variables: an explicitly validated REST request can never be invalidated by host-side `HA_CONTEXT_PROFILE`, `HA_CONTEXT_DETAIL`, `HA_CONTEXT_SECTIONS`, `HA_CONTEXT_INCLUDE_FILES`, `HA_CONTEXT_INCLUDE_STORAGE`, or `HA_CONTEXT_ON_BUDGET_EXCEEDED` state. The REST worker takes profile, section, source, and policy options exclusively from the validated request; when `maxBytes` is omitted, it falls back to `HA_CONTEXT_MAX_OUTPUT_BYTES`.
- The context generator no longer falls back to an implicit Home Assistant host: when `HA_URL` is unset, network collection stays disabled even when `HA_TOKEN` is set, so the token can never be transmitted to an implicit plain-HTTP endpoint. The server-side `RuntimeSettings` default changed the same way, so the REST context worker can no longer hand the generation child an implicit host either. Deployments that want API sources set `HA_URL` explicitly.
...
- The local `docker build` path (root `Dockerfile`) pins the same build toolchain as CI (`build` 1.6.0, `setuptools` 84.0.0, `wheel` 0.48.0), restoring local image builds after the build-tooling bump left the root image builder behind `pyproject.toml`'s exact build requirements.
