...
def register_area_tools(mcp, config_path: str, ha_url: str, ha_token: str):
    ...
    def _get_area_by_id_or_name(area_id: str) -> Optional[Dict]:
    ...
    def _get_integration_info(entry_id: str) -> str:
    ...
    async def get_area_devices_summary(area_id: str) -> str:
        ...
        """
        area = _get_area_by_id_or_name(area_id)

        if not area:
            # Provide list of available areas for suggestion
            areas = get_registry_areas(config_path)
            available = [{"id": a["id"], "name": a["name"]} for a in areas]
            return json.dumps(
                {
                    "success": False,
        ...
        # Check for entities assigned directly to area (without device)
        orphan_entities = [
