...
def _do_list_blueprints(config_path: str) -> str:
    """List all available blueprints from the filesystem."""
    try:
        blueprints_dir = Path(config_path) / "blueprints"
        if not os.path.isdir(blueprints_dir):
            return _error_response("blueprints directory not found")
...
def _do_get_blueprint_code(blueprint_path: str, config_path: str) -> str:
    """Read a single blueprint file from disk."""
    try:
        blueprints_dir = Path(config_path) / "blueprints"
        full_path = blueprints_dir / blueprint_path

        if not str(full_path.resolve()).startswith(str(blueprints_dir.resolve())):
            return _error_response("Access denied - path traversal detected")

        if not os.path.isfile(full_path):
            return _error_response(f"Blueprint '{blueprint_path}' not found")
        ...
        return _error_response(f"Error reading blueprint: {str(e)}")
...
def _do_get_blueprint_usage_summary(config_path: str) -> str:
    """Aggregate usage stats across all blueprints."""
    try:
...
def register_blueprint_tools(mcp, config_path) -> None:  # type: ignore[no-untyped-def]
    ...
    def list_blueprints() -> str:
        """[READ] Fetches list of all available blueprints.
        ...
        """
        try:
            return _do_list_blueprints(config_path)
        except Exception as e:
            return _error_response(str(e))

    @mcp.tool()
    def get_blueprint_code(blueprint_path: str) -> str:
        """[READ] Fetches full source YAML code of a blueprint.
        ...
        """
        try:
            return _do_get_blueprint_code(blueprint_path, config_path)
        except Exception as e:
            return _error_response(str(e))

    @mcp.tool()
    def get_blueprint_instances(blueprint_path: str) -> str:
        """[READ] Find all automations and scripts that use a specific blueprint.
        ...
        """
        try:
            return _do_get_blueprint_instances(blueprint_path, config_path)
        except Exception as e:
            return _error_response(str(e))

    @mcp.tool()
    def get_blueprint_usage_summary() -> str:
        """[READ] Summary of blueprint usage.

        Returns:
            JSON with summary (most used blueprints, unused, etc.)
        """
        try:
            return _do_get_blueprint_usage_summary(config_path)
        except Exception as e:
            return _error_response(str(e))
