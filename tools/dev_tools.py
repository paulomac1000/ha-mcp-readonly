...
def register_dev_tools(mcp, ha_url: str, ha_token: str, config_path: Optional[str] = None):
    """
    ...
    def test_template(template: str) -> str:
        """
        Tests a Jinja2 template in Home Assistant (like Developer Tools > Template).
    ...
    def test_templates_batch(templates: str) -> str:
        """
        BATCH TESTING - Tests multiple templates simultaneously.

        ~80% token savings when testing multiple templates.
        Instead of: test_template() × N
        ...
        if isinstance(templates_data, list):
            ...
            return json.dumps(
                {"success": False, "error": "Templates must be a JSON array or object"},
                indent=2,
            )
    ...
    def get_template_performance(template: str, iterations: int = 5) -> str:
        ...
        for _ in range(iterations):
            ...
            if not res["success"]:
                return json.dumps({"success": False, "error": res["error"]}, indent=2)
        ...
        if "states" in template and "states." not in template:
            complexity_score += 10  # iterating all states
        ...
        return json.dumps(
            {
                ...
                "benchmark": {
                    ...
                    "std_dev_ms": round(
                        (sum((t - avg_time) ** 2 for t in times) / len(times)) ** 0.5, 2
                    ),
                },
                "analysis": {
            ...
            for idx, trigger in enumerate(trigger_data):
                ...
                if platform == "state":
                    ...
                    if "value_template" not in trigger:
                        issues.append(
                            f"Trigger {idx}: 'template' platform requires 'value_template'"
                        )
                    else:
                    ...
                    if "entity_id" not in trigger or "zone" not in trigger:
                        issues.append(
                            f"Trigger {idx}: 'zone' platform requires 'entity_id' and 'zone'"
                        )
                    if "event" not in trigger:
    ...
    def check_entity_exists(entity_id: str) -> str:
        """
        Quickly checks if entity exists in the system.
    ...
    def check_entities_batch(entity_ids: str) -> str:
        """
        BATCH CHECKING - Checks multiple entities simultaneously.

        ~85% token savings when checking multiple entities.
        Instead of: check_entity_exists() × N
    ...
    def test_service_call(
        ...
        """
        Validates whether a service call is correct (WITHOUT EXECUTING!).
            ...
            return json.dumps(
                {
                    ...
                    "available_domains": sorted([d["domain"] for d in services_result["data"]])[
                        :20
                    ],
                },
                indent=2,
    ...
    def diagnose_entity(entity_id: str) -> str:
        ...
        # 2. Get info from registry (if config_path available)
        device_id = None
        area_id = None
                    ...
                    with open(registry_path, "r") as f:
                        ...
                        for entity in registry.get("data", {}).get("entities", []):
                            if entity.get("entity_id") == entity_id:
                                ...
                                # 3. Find related entities (same device)
                                if device_id:
                                            ...
                                            result["related_entities"].append(
                                                {
                                                    ...
                                                    "disabled": other.get("disabled_by")
                                                    is not None,
                                                }
                                            )
                                break
        ...
        if last_changed and last_updated:
            try:
                ...
                if time_since_change > 86400:  # 24h
                    result["issues"].append(
                        {
                            "severity": "warning",
    ...
    def diagnose_template(entity_id: str) -> str:
        ...
        # 1. Find template in config_entries (if config_path available)
        template_code = None
        ...
        if not template_code:
            # Try to get from current state attributes
            state_result = make_ha_request(ha_url, ha_token, f"/api/states/{entity_id}")
        ...
        # 2. Find referenced entities (regex)
        ...
        if not result["issues"]:
            ...
            if any("NOT_FOUND" in str(status) for status in result["entity_status"].values()):
                result["recommendations"].append(
                    "Remove or fix references to non-existent entities"
                )
    ...
    def diagnose_energy_setup() -> str:
        """
        CONTEXTUALIZED DIAGNOSTICS - Energy configuration diagnostics (especially for G12w tariff).
        ...
        # 2. Check energy automations (if config_path available)
        if config_path:
            try:
                import yaml
        ...
        # 6. Recommendations
        if result["tariff"] == "G12w detected":
            result["recommendations"].append(
                {
                    "priority": "info",
                    "message": "G12w tariff detected - remember peak hours: 06-13 and 15-22 (workdays)",
                }
            )
            result["recommendations"].append(
                {
                    "priority": "medium",
                    "message": "Consider automations for large consumers outside peak hours (savings ~250 PLN/month)",
                }
            )
        else:
            result["recommendations"].append(
                {
                    "priority": "high",
                    "message": "G12w tariff configuration not detected - add binary_sensor to track peak hours",
                }
            )
