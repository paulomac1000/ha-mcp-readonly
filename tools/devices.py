...
def register_device_tools(mcp, config_path: str, ha_url: str, ha_token: str):
    ...
    def _get_device_by_id(device_id: str) -> Optional[Dict]:
        """Find device by id."""
    ...
    def _get_config_entry_info(entry_id: str) -> Dict:
    ...
    def _get_area_name(area_id: str) -> Optional[str]:
        """Get area name by id."""
        if not area_id:
            return None
    ...
    async def get_device_details(device_id: str) -> str:
        """
        Fetches device details with full context.
            ...
            return json.dumps(
                {
                    ...
                    "suggestion": "Use search_devices() to find valid device_id",
                },
                indent=2,
            )
        ...
        # Build entities list with states
        entities_list = []
        entities_summary = {
        ...
        # Sort entities: enabled first, then by entity_id
        entities_list.sort(key=lambda x: (x.get("disabled_by") is not None, x.get("entity_id", "")))

        # Get config entry info
        config_entries_info = []
        for entry_id in device.get("config_entries", []):
            config_entries_info.append(_get_config_entry_info(entry_id))

        # Get via_device info if present
        via_device_info = None
        if device.get("via_device_id"):
            via_device = _get_device_by_id(device.get("via_device_id"))
            if via_device:
                via_device_info = {
        ...
        result = {
            ...
            "area_name": _get_area_name(device.get("area_id")),
            ...
            "entities": entities_list[:30],  # Limit to 30
        }
    ...
    async def get_device_entities(
        ...
        if not device:
            return json.dumps(
                {"success": False, "error": f"Device '{device_id}' not found"}, indent=2
            )
    ...
    async def search_devices(
        ...
        # Build entry_id -> domain map if filtering by domain
        entry_domain_map = {}
        if domain:
        ...
        for device in devices:
            ...
            if with_entities_count:
                device_entities = [
                    e for e in all_entities if e.get("device_id") == device.get("id")
                ]
        ...
        return json.dumps(
            {
                ...
                "devices": results[:50],  # Limit to 50
            },
            indent=2,
            ensure_ascii=False,
        )
    ...
    async def get_devices_by_area(area_id: str) -> str:
        ...
        if not area:
            available_areas = [{"id": a["id"], "name": a["name"]} for a in areas]
            return json.dumps(
                {
                    "success": False,
    ...
    async def device_get_wifi_status(device_id: str) -> str:
        ...
        # Check if device is Tasmota/OpenBK based on manufacturer/model
        manufacturer = (device.get("manufacturer") or "").lower()
        model = (device.get("model") or "").lower()

        is_tasmota = "tasmota" in manufacturer or "tasmota" in model
        is_openbk = "openbk" in manufacturer or "openbeken" in manufacturer

        if not is_tasmota and not is_openbk:
            # Not a WiFi device we can query - try HA sensors
            pass

        # Get device entities to find WiFi-related sensors
        all_entities = get_registry_entities(config_path)
        device_entities = [e for e in all_entities if e.get("device_id") == device_id]
        ...
        # Look for WiFi-related sensors in HA
        wifi_related_ids = ["rssi", "wifi", "signal", "ssid", "ip", "uptime", "status"]

        for entity in device_entities:
            entity_id = entity.get("entity_id", "").lower()

            # Check if entity is WiFi-related
            if any(wifi_id in entity_id for wifi_id in wifi_related_ids):
                ...
                if state_response.get("success"):
                    ...
                    # Check device class for additional info
        ...
        # Determine connection state based on RSSI
        ...
        return json.dumps({"success": True, "wifi_status": wifi_data}, indent=2, ensure_ascii=False)
